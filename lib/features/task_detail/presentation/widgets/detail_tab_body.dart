import 'package:flutter/material.dart';

import '../../../../core/domain/entities/ticket.dart';
import '../detail_providers.dart';
import 'comments_tab.dart';
import 'development_tab.dart';
import 'original_tab.dart';
import 'translation_tab.dart';

/// Renders the active detail tab's content.
class DetailTabBody extends StatelessWidget {
  const DetailTabBody({super.key, required this.ticket, required this.tab});
  final Ticket ticket;
  final DetailTab tab;

  @override
  Widget build(BuildContext context) {
    switch (tab) {
      case DetailTab.original:
        return OriginalTab(ticket: ticket);
      case DetailTab.translation:
        return TranslationTab(ticket: ticket);
      case DetailTab.comments:
        return CommentsTab(ticketId: ticket.id);
      case DetailTab.development:
        return DevelopmentTab(ticket: ticket);
    }
  }
}
